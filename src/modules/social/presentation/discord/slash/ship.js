...
module.exports = {
  ...
  async execute(interaction) {
    ...
    const shipName =
      user1.username.slice(0, user1.username.length / 2) +
    ...
    try {
      const avatar1 = await loadImage(
        user1.displayAvatarURL({ format: 'png', size: 256, forceStatic: true }),
      );
      const avatar2 = await loadImage(
        user2.displayAvatarURL({ format: 'png', size: 256, forceStatic: true }),
      );
